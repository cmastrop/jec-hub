...
export default function CategorySongsPage() {
  ...
  const fetchSongs = useCallback(async () => {
    ...
    try {
      ...
      if (!artistFilter && !search && !activeKey) {
        const uniqueArtists = [
          ...new Set(
            (data.songs || [])
              .map((s: Song) => s.artist)
    ...
    setLoading(false);
  }, [categoryName, search, activeKey, artistFilter]);
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center gap-2 text-sm text-gray-500 mb-1">
          ...
          <Link href="/canciones/categorias" className="hover:text-primary">
            Categorias
          </Link>
          <ChevronRight className="w-3 h-3" />
